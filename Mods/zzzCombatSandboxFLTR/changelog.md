...
From release 3.2.0.2.0.0, the format is based on [Keep a Changelog](https://keepachangelog.com/en/1.1.0/).

From release 3.2.0.2.0.0 this project adheres to [Semantic Versioning](https://semver.org/spec/v2.0.0.html).
...
- Code: Byteblazar (custom code adding the Combine Station to the 'No Repair' challenge).
- Bug reports: bloodaxis (crosshair), RozzieHarris and D4CTO (.45 ACP Deadeye weapon damage).
- Feature suggestions: Boogeyman (x10 Stack All Items (60k Cap)), Fegeer2020 (remove vanilla handguns and knives from toilet loot), oktanion (silenced weapons), 3vTAC (dual weapons).

### Added
- Double mod version system: 4 integers (for <Version> official tag in ModInfo.xml) and 6 integers (for <InternalVersion> unofficial tag in ModInfo.xml).
- Option: disable custom gloves.
- Support for x10 Stack All Items (60k Cap) mod.
- Option: Combine Station in the 'No Repair' challenge.
- Option: Separate option for tools in the 'No Repair' challenge.
- Option: Extend ammo45ACPEnhanced compatibility to all .45 ACP weapons.
- Option: Remove vanilla handguns and knives from toilet loot. 
- Option: Support for ammoShotgunHellFire from LittleRedSonja T2 into T3 grade weapons to vanilla and IZY Classic shotguns.
- IZY Classic pistols added to groupNightstand02.
- Option: Vanilla and IZY Classic weapons can now be set to use quieter (silenced) shooting sounds.
- Option: Align the number of mods slots in vanilla weapons and tools on the item's quality level.
- Option: Remove dual weapons.
- Option: T2 and T3 dual weapons are more expensive to craft.

### Changed
- Set CrosshairOnAim and CrosshairUpAfterShot to true for all IZY Classic weapons and weapon mods to fix a conflict with the v3 crosshair options.
- IZYgunT3TechnicalNailmachinegun recipe aligned with other T3 weapons.
- modGunLaserSight no longer compatible with weapons not set up to display the laser beam (IZYgunT145TACSMGMAC10, IZYgunT0SNIPERpipeAR15Broken15Junk, IZYgunTminus1MusketM1777Rifle, IZYgunT0PistolLiberatorImprovisedPistol)
- Toilet pistols: groupToilet02 changed to groupToilet03.
- Vanilla sound options moved to 6. Misc. Options tab.

### Fixed

- Missing modslots to IZYgunT0PistolLiberatorImprovisedPistol and IZYgunTminus1MusketFlintlockPistol.
- Fixed .45 ACP Deadeye weapon damage 
RozzieHarris and D4CTO
- gunShotgunT0PipeShotgun uses the same silencer sound as other shotguns.

### Removed

- T1 Pistols in Military Footlockers: removed loot_prob_template.

### Known Issues
- Option: With auto-reload on for all weapons, the second and ff bolts are not visible when using gunBowT3CompoundCrossbow.

## [3.0.1.0.0] - (2026-07-16)

### Added
- Compatibility with 7 Days To Die v3.
- Option: the Lever-Action Rifle is compatible with the Tube Extender Mod.
- Support for V3_OakravenAmmoPress.
- ammoRocketRPGTraning and ammo25mmGrenadeTraining get thrownAmmoPipeBomb particle index value.
- perkPenetrator for ammo556mmBulletBall and ammo556mmArmorPiercing.

### Changed
- Updated perkPenetrator for ammo45ACPEnhanced, ammo45ACPArmorpiecing, ammoCrossbowLightBoltIron, ammoCrossbowLightBoltSteelAP.
- Single ammo craft times aligned on vanilla.
- V3_OakravenAmmoPress single ammo craft time divided by 2 for IZY Classic and vanilla ammo.

### Removed
- Legacy code for modGunBarrelExtender and modShotgunSawedOffBarrel from 7D1.0_Izayo_Visible_Mods_mod\item_modifiers.xml.
- Tube Extender mod options.
- ammoShotgunShellPremium unlocked by perkShotgunMessiahBreachingSlugs.
- Custom Explosion Particle Loader effects (not available for 7 Days To Die v3).
- IZYgunT5HeavyWeaponM2flameThrower (temporarily) 
